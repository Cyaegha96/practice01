	...
	<c:choose>
		...
		<c:otherwise>
							...
							<c:forEach var="dto" items="${list}">
								<div class="row board-item">
									...
									<div class="col-4 titles">
										<a href="/detail.board?seq=${dto.seq}" style="text-decoration: none; color: inherit;">${dto.title}</a>
									</div>
			...
			<script>
				...
				// 앵커 태그 달기
				if(needPrev) {
					$(".page").append("<a href='/list.board?cpage=" + (startNavi-1) + "'>< </a>");
				}

				for(let i = startNavi; i<= endNavi; i++) {
					$(".page").append("<a href='/list.board?cpage=" + i + "'>" + i + "</a> ");
				}
				if(needNext) {
					$(".page").append(" <a href='/list.board?cpage=" + (endNavi+1) + "'> > </a>");
				}
